...
def substep_assignment(nodes_df=None, weighted_edges_df=None, od_ss=None, quarter_demand=None, assigned_demand=None, quarter_counts=4, agent_travel_time_dict=None):
    ...
    # all_path_vol = dict()
    weighted_edges_df['volume_substep'] = 0
    all_path_vol = weighted_edges_df['volume_substep'].T.to_dict()
    ...
    for p in paths:
        ...
        for edge_s, edge_e in zip(p, p[1:]):
            ...
            if p_dist > 3600/quarter_counts:
                ...
                break
        agent_travel_time_dict[agent_ids[path_i]] += p_dist
        path_i += 1

    all_path_vol_df = pd.DataFrame.from_dict(all_path_vol, orient='index', columns=['vol_ss'])
    all_path_vol_df = all_path_vol_df.loc[all_path_vol_df['vol_ss']>0]
    ...
    return new_edges_df, od_residual_ss_list, agent_travel_time_dict
...
def read_od(demand_files=None):
    ...
    agent_travel_time_dict = {getattr(agent, 'agent_id'): 0 for agent in od_all.itertuples()}
    t_od_1 = time.time()
    logging.info('{} sec to read {} OD pairs'.format(t_od_1-t_od_0, od_all.shape[0]))
    return od_all, agent_travel_time_dict
...
def assignment(quarter_counts=4, substep_counts=15, substep_size=100000, network_file_nodes=None, network_file_edges=None, demand_files=None, simulation_outputs=None, scen_nm=None, hour_list=None, quarter_list=None, cost_factor=None):
    ...
    ### OD processing
    od_all, agent_travel_time_dict = read_od(demand_files=demand_files)
        ...
        for hour in hour_list:
            ...
            for quarter in quarter_list:
                ...
                for ss_id in substep_ids:
                    ...
                    edges_df, od_residual_ss_list, agent_travel_time_dict = substep_assignment(nodes_df=nodes_df, weighted_edges_df=weighted_edges_df, od_ss=od_ss, quarter_demand=quarter_demand, assigned_demand=assigned_demand, quarter_counts=quarter_counts, agent_travel_time_dict=agent_travel_time_dict)
    ...
    agent_travel_time_df = pd.DataFrame.from_dict(agent_travel_time_dict, orient='index', columns=['travel_time'])
    agent_travel_time_df.loc[agent_travel_time_df['travel_time']>0].to_csv(simulation_outputs+'/agent_travel_time.csv')
...
if __name__ == "__main__":
    status = main(hour_list=list(range(3, 4)), quarter_list=[0,1,2,3], scen_nm='')
