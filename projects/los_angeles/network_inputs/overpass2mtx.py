...
def populate_attributes(w):
    ...
    drivable_twoway_default = ['primary', 'primary_link', 'secondary', 'secondary_link', 'tertiary', 'tertiary_link', 'unclassified', 'unsurfaced', 'residential', 'living_street', 'service']
    ...
    if 'lanes' in w['tags'].keys(): w_lanes = int(w['tags']['lanes'].split(';')[0]) ### sometimes lanes are "2;5". Only keep the first number
...
def merge_by_street(ways):
    ...
    ### Connect two roads if (1) they have the same street name; (2) they have a common node; (3) no other street has this common node; (4) they have same speed limit.
    ...
    for clean_round in range(3):
        ...
        melt_df = process_ways_df.melt(id_vars = ['osmid', 'name', 'oneway', 'speed_limit'], value_vars=['s_node', 'e_node'], value_name='end_node')
        ...
        osmid_pairs = end_node_count_df.loc[(
            ...
            end_node_count_df['speed_limit'][0]==end_node_count_df['speed_limit'][1]) & (
...
def osm_to_json(output_csv=False):
    ...
    osm_data = json.load(open(absolute_path+'/target_scag.osm'))
    ...
    if output_csv:
        intersection_nodes_df.to_csv(absolute_path+'/unsimplified_nodes.csv', index=False)
        split_ways.to_csv(absolute_path+'/unsimplified_edges.csv', index=False)
        # sys.exit(0)
...
def convert_to_mtx(g):
    ...
    edge_attributes_df['fft'] = edge_attributes_df.apply(lambda row: row['length']/row['maxmph']*2.23694 * 1.2 #+ row['traffic_signals_delay'] + row['crossings_stops_delay']
    , axis=1) # 2.23694 is to convert mph to m/s;
    
    ### Convert to mtx
    wgh = edge_attributes_df['fft']
    g_coo = sp.coo_matrix((wgh, (row, col)), shape=(g.vcount(), g.vcount()))
    print(g_coo.shape, len(g_coo.data))
    # sio.mmwrite(absolute_path+'/data/bolinas/network_sparse.mtx', g_coo)
    # g_coo = sio.mmread(absolute_path+'/../data/{}/network_sparse.mtx'.format(folder))

    node_attributes_df.to_csv(absolute_path+'/osm_nodes_scag.csv', index=False)
    edge_attributes_df.to_csv(absolute_path+'/osm_edges_scag.csv', index=False)


if __name__ == '__main__':
    nodes_df, edges_df = osm_to_json(output_csv=False)
